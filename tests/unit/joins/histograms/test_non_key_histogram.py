...
class TestHistogramMethod(unittest.TestCase):
    ...
    def test_NonKeyTopKHistogram_large_continuous_less_bin(self):
        ...
        data = pd.read_csv("data/stats/posts.csv")[["PostTypeId"]]

        # print(data)
        hist = NonKeyTopKHistogram(n_top_k=10, n_bins=50, n_categorical=20)
        hist.fit(data, headers=["PostTypeId"])

        error = 4
        domain = Domain(mins=1, left=True, right=True)
        self.assertEqual(91976, hist.selectivity(domain, frac=False))
        domain = Domain(mins=0, left=True, right=True)
        self.assertTrue(abs(91976 - hist.selectivity(domain, frac=False)) < error)
        domain = Domain(mins=5, left=False, right=True)
        self.assertTrue(abs(14 - hist.selectivity(domain, frac=False)) < error)
        domain = Domain(mins=5, left=True, right=True)
        self.assertTrue(abs(654 - hist.selectivity(domain, frac=False)) < error)

        domain = Domain(mins=1, maxs=1, left=True, right=True)
        self.assertEqual(42921, hist.selectivity(domain, frac=False))

        domain = Domain(mins=3, maxs=3, left=True, right=True)
        self.assertEqual(6, hist.selectivity(domain, frac=False))

        domain = Domain(mins=7, maxs=7, left=True, right=True)
        self.assertEqual(5, hist.selectivity(domain, frac=False))
