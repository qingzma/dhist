...
from joins.fast_interp import interp2d
...
for ki, k in enumerate(ktest):
    ...
    for ni, n in enumerate(ntest):
        ...
        def test_function(x, y): return np.exp(
            x)*np.cos(y) + x*y**3/(1 + x + y)
        f = test_function(x, y) + 2 * \
            (np.random.rand(*x.shape)-0.5)*random_noise_size
        ...
        if k <= 5:
            st = time.time()
            interpolater = sp.interpolate.RectBivariateSpline(
                v, v, f, kx=k, ky=k)
...
ax.plot(ntest, sp_errors[:, 0], color='black',
        linestyle='--', label='Scipy, Linear')
ax.plot(ntest, sp_errors[:, 1], color='blue',
        linestyle='--', label='Scipy, Cubic')
ax.plot(ntest, sp_errors[:, 2], color='purple',
        linestyle='--', label='Scipy, Qunitic')
...
fig, ax = plt.subplots(1, 1)
ax.plot(ntest, my_setup_time[:, 0], color='black',
        label='fast_interp, Linear')
ax.plot(ntest, my_setup_time[:, 1], color='blue',   label='fast_interp, Cubic')
ax.plot(ntest, my_setup_time[:, 2],
        color='purple', label='fast_interp, Qunitic')
ax.plot(ntest, my_setup_time[:, 3], color='red',
        label='fast_interp, Septic')
ax.plot(ntest, my_setup_time[:, 4], color='green',  label='fast_interp, Nonic')
ax.plot(ntest, sp_setup_time[:, 0], color='black',
        linestyle='--', label='Scipy, Linear')
ax.plot(ntest, sp_setup_time[:, 1], color='blue',
        linestyle='--', label='Scipy, Cubic')
ax.plot(ntest, sp_setup_time[:, 2], color='purple',
        linestyle='--', label='Scipy, Qunitic')
ax.set_xlabel(r'$n$')
...
ax.plot(ntest, my_eval_time[:, 2], color='purple',
        label='fast_interp, Qunitic')
...
ax.plot(ntest, sp_eval_time[:, 0], color='black',
        linestyle='--', label='Scipy, Linear')
ax.plot(ntest, sp_eval_time[:, 1], color='blue',
        linestyle='--', label='Scipy, Cubic')
ax.plot(ntest, sp_eval_time[:, 2], color='purple',
        linestyle='--', label='Scipy, Qunitic')
ax.set_xlabel(r'$n$')
ax.set_ylabel('Time (ms)')
ax.set_xscale('log')
...
fig, ax = plt.subplots(1, 1)
ax.plot(ntest, sp_eval_time[:, 0]/my_eval_time[:, 0],
        color='black',  label='Linear')
ax.plot(ntest, sp_eval_time[:, 1] /
        my_eval_time[:, 1], color='blue',   label='Cubic')
ax.plot(ntest, sp_eval_time[:, 2]/my_eval_time[:, 2],
        color='purple', label='Qunitic')
ax.axhline(1.0, color='gray')
ax.set_xlabel(r'$n$')
ax.set_ylabel('Ratio (scipy/fast_interp)')
ax.set_xscale('log')
ax.set_yscale('log')
...
def test_function(x, y): return np.exp(x)*np.cos(y) + x*y**3/(1 + x + y)
...
for n in [20, 40, 80, 160]:
    ...
    def test_function(x, y): return np.exp(np.sin(x))*np.exp(y)
    f = test_function(x, y)
    fa = test_function(xo, yo)

    interpolater = interp2d([ax, ay], [bx, by], [
                            xh, yh], f, k=1, p=[True, False])
    fe = interpolater(xo, yo)
    err = np.abs(fe - fa).max()
...
for n in [20, 40, 80, 160]:
    ...
    def test_function(x, y): return np.exp(x)*np.exp(np.sin(y))
    f = test_function(x, y)
    fa = test_function(xo, yo)

    interpolater = interp2d([ax, ay], [bx, by], [
                            xh, yh], f, k=3, p=[False, True])
    fe = interpolater(xo, yo)
    err = np.abs(fe - fa).max()
    print('...Error is: {:0.1e}'.format(err))
...
for n in [20, 40, 80, 160]:
    ...
    def test_function(x, y): return np.exp(np.sin(x))*np.cos(y)
    f = test_function(x, y)
    fa = test_function(xo, yo)
