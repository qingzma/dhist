...
from joins.imdb.prepare_data import process_imdb_data
from joins.stats.schema import get_stats_relevant_attributes
from joins.imdb.schema import get_imdb_relevant_attributes

from joins.table_top_k import TableContainerTopK


def train_imdb_job_light_topk(args):
    ...
    if os.path.exists(model_path):
        ...
        schema, all_keys, equivalent_keys, table_keys = process_imdb_data(
            dataset, data_path, model_folder, kernel=None
        )
        join_keys, relevant_keys, counters = get_imdb_relevant_attributes(schema)
    ...
    if os.path.exists(model_path + ".dom"):
        ...
        with open("models/job-light/" + model_name + ".pkl", "rb") as f:
            model = pickle.load(f)
