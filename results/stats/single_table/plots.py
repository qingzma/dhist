...
def plot_accuracy():
    ...
    card = read_from_csv("results/stats/single_table/card_s.csv", "card")
    ...
    logbins = np.logspace(
        np.log10(min(min(re_card), min(re_postgres), min(re_bayescard), min(re_wjsample), min(re_deepdb))),
        np.log10(max(max(re_card), max(re_postgres), max(re_bayescard), max(re_wjsample), max(re_deepdb))),
        301,
    )
    plt.xscale("log")
    plt.hist(re_card, bins=logbins, label="card", alpha=0.5)
    plt.hist(re_postgres, bins=logbins, label="postgres", alpha=0.5)
    plt.hist(re_bayescard, bins=logbins, label="BayesCard", alpha=0.5)
    plt.hist(re_wjsample, bins=logbins, label="WJSample", alpha=0.5)
    plt.hist(re_deepdb, bins=logbins, label="DeepDB", alpha=0.5)
    plt.legend()
    plt.show()
...
def plot_times():
    ...
    bayescard = read_from_csv("results/stats/single_table/bayescard.csv", "bayescard-time")
    ...
    logbins = np.logspace(
        np.log10(min(min(card), min(postgres), min(bayescard), min(wjsample), min(deepdb), min(truths))),
        np.log10(max(max(card), max(postgres), max(bayescard), max(wjsample), max(deepdb), max(truths))),
        301,
    )
    plt.xscale("log")
